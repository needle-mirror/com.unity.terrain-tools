...
#ifndef UNITY_TERRAIN_TOOL_NOISE_StrataBillow_INC
#define UNITY_TERRAIN_TOOL_NOISE_StrataBillow_INC
...
float noise_StrataBillow_Raw( float pos, StrataFractalInput fractalInput )
{
    float prev = 0;
    float n = 0;
...
float noise_StrataBillow_Raw( float2 pos, StrataFractalInput fractalInput )
{
    float prev = 0;
    float n = 0;
...
float noise_StrataBillow_Raw( float3 pos, StrataFractalInput fractalInput )
{
    float prev = 0;
    float n = 0;
...
float noise_StrataBillow_Raw( float4 pos, StrataFractalInput fractalInput )
{
    float prev = 0;
    float n = 0;
...
/*=========================================================================
    ...
    StrataBillow Noise Functions - Fractal, Warped

=========================================================================*/

float noise_StrataBillow( float pos, StrataFractalInput fractalInput )
{
    if(fractalInput.warpIterations > 0)
    {
        ...
        {
            float q = noise_StrataBillow_Raw( pos + fractalInput.warpOffsets.x, fractalInput );
            prev = pos;
    ...
    float h = noise_StrataBillow_Raw( pos, fractalInput );
...
float noise_StrataBillow( float2 pos, StrataFractalInput fractalInput )
{
    if(fractalInput.warpIterations > 0)
    {
        float2 prev = 0;
        ...
        for ( float i = 0; i < warpIterations; ++i )
        {
            float2 q = float2( noise_StrataBillow_Raw( pos, fractalInput ),
                            noise_StrataBillow_Raw( pos + fractalInput.warpOffsets.xy, fractalInput ) );
            prev = pos;
...
float noise_StrataBillow( float3 pos, StrataFractalInput fractalInput )
{
    if(fractalInput.warpIterations > 0)
    {
        float3 prev = 0;
        ...
        for ( float i = 0; i < warpIterations; ++i )
        {
            float3 q = float3( noise_StrataBillow_Raw( pos.xyz, fractalInput ),
                        noise_StrataBillow_Raw( pos.xyz + fractalInput.warpOffsets.xyz, fractalInput ),
                        noise_StrataBillow_Raw( pos.xyz + float3( fractalInput.warpOffsets.x, fractalInput.warpOffsets.y, 0 ), fractalInput ) );
            prev = pos;
            pos = pos + fractalInput.warpStrength * q;
        }
...
float noise_StrataBillow( float4 pos, StrataFractalInput fractalInput )
{
    float h = noise_StrataBillow_Raw( pos, fractalInput );
...
#endif // UNITY_TERRAIN_TOOL_NOISE_StrataBillow_INC