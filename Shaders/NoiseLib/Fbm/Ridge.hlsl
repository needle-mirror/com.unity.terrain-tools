...
#ifndef UNITY_TERRAIN_TOOL_NOISE_FbmRidge_INC
#define UNITY_TERRAIN_TOOL_NOISE_FbmRidge_INC
...
float noise_FbmRidge_Raw( float pos, FbmFractalInput fractalInput )
{
    float prev = 0;
    float n = 0;
...
float noise_FbmRidge_Raw( float2 pos, FbmFractalInput fractalInput )
{
    float prev = 0;
    float n = 0;
...
float noise_FbmRidge_Raw( float3 pos, FbmFractalInput fractalInput )
{
    float prev = 0;
    float n = 0;
...
float noise_FbmRidge_Raw( float4 pos, FbmFractalInput fractalInput )
{
    float prev = 0;
    float n = 0;
...
/*=========================================================================
    ...
    FbmRidge Noise Functions - Fractal, Warped

=========================================================================*/

float noise_FbmRidge( float pos, FbmFractalInput fractalInput )
{
    if(fractalInput.warpIterations > 0)
    {
        ...
        {
            float q = noise_FbmRidge_Raw( pos + fractalInput.warpOffsets.x, fractalInput );
            prev = pos;
    ...
    float f = noise_FbmRidge_Raw( pos, fractalInput );

    return f;
}

float noise_FbmRidge( float2 pos, FbmFractalInput fractalInput )
{
    if(fractalInput.warpIterations > 0)
    {
        float2 prev = 0;
        ...
        for ( float i = 0; i < warpIterations; ++i )
        {
            float2 q = float2( noise_FbmRidge_Raw( pos, fractalInput ),
                            noise_FbmRidge_Raw( pos + fractalInput.warpOffsets.xy, fractalInput ) );
            prev = pos;
...
float noise_FbmRidge( float3 pos, FbmFractalInput fractalInput )
{
    if(fractalInput.warpIterations > 0)
    {
        float3 prev = 0;
        ...
        for ( float i = 0; i < warpIterations; ++i )
        {
            float3 q = float3( noise_FbmRidge_Raw( pos.xyz, fractalInput ),
                        noise_FbmRidge_Raw( pos.xyz + fractalInput.warpOffsets.xyz, fractalInput ),
                        noise_FbmRidge_Raw( pos.xyz + float3( fractalInput.warpOffsets.x, fractalInput.warpOffsets.y, 0 ), fractalInput ) );
            prev = pos;
            pos = pos + fractalInput.warpStrength * q;
        }
...
float noise_FbmRidge( float4 pos, FbmFractalInput fractalInput )
{
    float f = noise_FbmRidge_Raw( pos, fractalInput );
...
#endif // UNITY_TERRAIN_TOOL_NOISE_FbmRidge_INC